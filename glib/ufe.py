...
from .dgii_health_checker import (
    ErrorType,
    ServiceStatus,
    execute_with_retry,
    get_health_checker,
...
def validar_rnc(rnc: str) -> tuple[bool, str]:
    """
    ...
    Examples:
        >>> validar_rnc("123456789")
        (True, "RNC válido")
        >>> validar_rnc("ABC123")
        (False, "RNC formato inválido: ABC123 (debe ser 9 u 11 dígitos)")
    """
    if not rnc:
    ...
    # Solo dígitos, 9 u 11 caracteres
    if not re.match(r"^\d{9}$|^\d{11}$", rnc):
        return False, f"RNC formato inválido: {rnc} (debe ser 9 u 11 dígitos)"

    # No todo ceros
    if rnc == "0" * len(rnc):
        return False, "RNC no puede ser todo ceros"

    # TODO: Validar dígito verificador según algoritmo DGII

    return True, "RNC válido"


def validar_encf(encf: str, tipo_esperado: int = None) -> tuple[bool, str]:
    """
    Valida formato de eNCF (Número de Comprobante Fiscal Electrónico).

    Formato: E[TipoComprobante][09 dígitos secuenciales]
    Ejemplo: E310000000001 (Factura Crédito Fiscal)

    Args:
        encf: eNCF a validar
        tipo_esperado: Tipo de comprobante esperado (opcional)

    Returns:
        (es_valido, mensaje_error)

    Examples:
        >>> validar_encf("E310000000001")
        (True, "eNCF válido")
        >>> validar_encf("E990000000001")
        (False, "Tipo de comprobante '99' inválido")
        >>> validar_encf("E310000000001", tipo_esperado=32)
        (False, "Tipo esperado 32, encontrado 31")
    """
    if not encf:
        return False, "eNCF vacío"

    encf = encf.strip()

    if len(encf) != 13:
        return False, f"eNCF debe tener 13 caracteres (tiene {len(encf)})"

    if not encf.startswith("E"):
        return False, "eNCF debe comenzar con 'E'"

    # Tipos válidos según DGII
    ...
    if tipo not in tipos_validos:
        return (
            False,
            f"Tipo de comprobante '{tipo}' inválido. Válidos: {', '.join(tipos_validos)}",
        )

    # Verificar que el tipo coincida con el esperado
    if tipo_esperado is not None and tipo != str(tipo_esperado).zfill(2):
        return False, f"Tipo esperado {tipo_esperado}, encontrado {tipo}"

    # Secuencia debe ser numérica
    secuencia = encf[3:]
    if not secuencia.isdigit():
        return False, "Secuencia debe ser numérica"

    # Secuencia no puede ser cero
    if int(secuencia) == 0:
        return False, "Secuencia no puede ser 000000000"

    return True, "eNCF válido"


def validar_montos_factura(
    tipo_ecf: str,
    ...
    """
    Valida coherencia y rangos de montos en factura.

    Args:
        tipo_ecf: Tipo de comprobante (31, 32, etc.)
        monto_subtotal: Subtotal antes de impuestos
        monto_itbis: Monto de ITBIS (18%)
        monto_total: Monto total de la factura
        descuento: Descuento aplicado (opcional)
        propina: Propina incluida (opcional)
        es_resumen: Si es un resumen de facturas

    Returns:
        (es_valido, mensaje_error)

    Raises:
        ValueError: Si algún parámetro tiene tipo incorrecto
    """
    # Convertir a Decimal si es necesario
    try:
        monto_subtotal = Decimal(str(monto_subtotal))
        monto_itbis = Decimal(str(monto_itbis))
        monto_total = Decimal(str(monto_total))
        descuento = Decimal(str(descuento))
        propina = Decimal(str(propina))
    except (ValueError, TypeError) as e:
        return False, f"Error de conversión de montos: {e}"

    # 1. Montos no negativos
    if any(m < 0 for m in [monto_subtotal, monto_itbis, monto_total]):
        return False, "Los montos no pueden ser negativos"

    if descuento < 0 or propina < 0:
        return False, "Descuento y propina no pueden ser negativos"

    # 2. Total mínimo
    if monto_total < Decimal("0.01"):
        return False, "El monto total debe ser mayor a 0"

    # 3. Coherencia aritmética (con tolerancia de 0.02 por redondeo)
    total_calculado = monto_subtotal + monto_itbis - descuento + propina
    diferencia = abs(total_calculado - monto_total)

    if diferencia > Decimal("0.02"):
        return False, (
            f"Total incoherente. "
            f"Calculado: RD${total_calculado:.2f}, "
            f"Recibido: RD${monto_total:.2f}, "
            f"Diferencia: RD${diferencia:.2f}"
        )

    # 4. Validar ITBIS (18% aprox, con tolerancia para exenciones)
    if monto_subtotal > 0:
        itbis_esperado = monto_subtotal * Decimal("0.18")
        diferencia_itbis = abs(itbis_esperado - monto_itbis)

        # Tolerancia: hasta 20% para casos de productos exentos
        if diferencia_itbis > (monto_subtotal * Decimal("0.20")):
            logger.warning(
                f"ITBIS fuera de rango esperado. "
                f"Esperado: RD${itbis_esperado:.2f}, "
                f"Recibido: RD${monto_itbis:.2f}"
            )

    # 5. Descuento no mayor al subtotal
    if descuento > monto_subtotal:
        return (
            False,
            f"Descuento (RD${descuento:.2f}) no puede ser mayor al subtotal (RD${monto_subtotal:.2f})",
        )

    # 6. Propina razonable (máximo 25% del subtotal)
    if propina > monto_subtotal * Decimal("0.25"):
        return False, f"Propina (RD${propina:.2f}) excede el 25% del subtotal"

    # 7. ITBIS no puede ser mayor al subtotal
    if monto_itbis > monto_subtotal:
        return (
            False,
            f"ITBIS (RD${monto_itbis:.2f}) no puede ser mayor al subtotal (RD${monto_subtotal:.2f})",
        )

    # 8. Límites según tipo de comprobante
    if tipo_ecf == "32":  # Factura de Consumo
        if monto_total >= Decimal("250000") and es_resumen:
            return False, (
                f"Factura de consumo ≥ RD$250,000.00 no puede ser resumen. "
                f"Total: RD${monto_total:.2f}"
            )

    # 9. Monto máximo general (límite práctico)
    if monto_total > Decimal("999999999.99"):
        return False, "Monto total excede límite permitido (RD$999,999,999.99)"
...
def sanitize_for_log(data: str) -> str:
    ...
    - Passwords
    - RNC (muestra solo últimos 4 dígitos)
    - URLs con query params sensibles

    Args:
        data: String a sanitizar

    Returns:
        String sanitizado

    Examples:
        >>> sanitize_for_log("Bearer abc123def456")
        "Bearer ***REDACTED***"
        >>> sanitize_for_log("RNC: 123456789")
        "RNC: 12345**"
    """
    if not data or not isinstance(data, str):
        return data

    # Ocultar tokens Bearer
    data = re.sub(
        r"(bearer\s+|token[=:]?[\s]*)([a-zA-Z0-9\-._~+/]+=*)",
        r"\1***REDACTED***",
        data,
        flags=re.IGNORECASE,
    )
    ...
    # Ocultar passwords
    data = re.sub(
        r"(password[=:]?[\s]*)([^\s&]+)", r"\1***REDACTED***", data, flags=re.IGNORECASE
    )

    # Ocultar RNC completos (mostrar solo últimos 4 dígitos para debugging)
    data = re.sub(
        r"\b(\d{5,7})\d{2,4}\b",
    ...
    # Ocultar query params sensibles
    data = re.sub(
...
def sanitize_filename(filename: str, max_length: int = 255) -> str:
    ...
    Examples:
        ...
        "etcpasswd"
        >>> sanitize_filename("file<>name.xml")
        "filename.xml"
    """
    if not filename:
        return "unnamed"

    # Remover caracteres peligrosos (permitir solo alfanuméricos, guiones, guiones bajos, puntos)
    filename = re.sub(r"[^\w\-_\.]", "", filename)

    # Prevenir path traversal (eliminar secuencias ..)
    while ".." in filename:
        filename = filename.replace("..", "")
    filename = os.path.basename(filename)

    # Prevenir nombres peligrosos de Windows
    windows_reserved = [
        "CON",
        "PRN",
        "AUX",
...
def generar_ruta_segura(base_dir: str, subdirs: list, filename: str) -> str:
    """
    Genera ruta segura previniendo path traversal.

    Args:
        base_dir: Directorio base absoluto
        subdirs: Lista de subdirectorios
    ...
    Raises:
        ValueError: Si la ruta resultante está fuera de base_dir

    Examples:
        >>> generar_ruta_segura("/base", ["sub"], "file.xml")
        "/base/sub/file.xml"
        >>> generar_ruta_segura("/base", [], "../../etc/passwd")
        ValueError: Path traversal detectado
    """
    # Sanitizar filename
    safe_filename = sanitize_filename(filename)

    # Construir ruta
    path_parts = [base_dir] + subdirs + [safe_filename]
    target_path = os.path.abspath(os.path.join(*path_parts))

    # Verificar que está dentro de base_dir
    base_abs = os.path.abspath(base_dir)
    if not target_path.startswith(base_abs):
        raise ValueError(
            f"Path traversal detectado: {target_path} está fuera de {base_abs}"
        )

    return target_path


def escribir_xml_atomico(contenido: str, ruta_destino: str) -> bool:
    """
    Escribe XML de forma atómica usando rename.

    Args:
        contenido: Contenido del XML
        ruta_destino: Ruta final del archivo

    Returns:
        True si éxito, False en caso contrario

    Security:
        Previene race conditions usando archivo temporal + rename atómico
    """
    import tempfile

    # Crear archivo temporal en mismo directorio
    ...
    try:
        ...
        # Rename es atómico en la mayoría de sistemas
        ...
        if "temp_path" in locals() and os.path.exists(temp_path):
            try:
                ...
                pass
        logger.error(f"Error escribiendo XML atómicamente: {e}", exc_info=True)
        return False
...
def formato_fecha_seguro(
    ...
    if isinstance(fecha_obj, str):
        ...
        for f in formatos:
            try:
                ...
                logger.debug(f"Formato de fecha inválido: {fecha_str}, formato: {f}")
                continue  # Intenta siguiente formato
            except Exception as e:
                logger.error(
                    f"Error inesperado procesando fecha {fecha_str}: {e}", exc_info=True
                )
                return ""
    ...
    try:
        ...
        logger.debug(f"Objeto {type(fecha_obj)} no tiene método strftime: {e}")
        return ""
    except Exception as e:
        logger.error(
            f"Error inesperado convirtiendo fecha {fecha_obj}: {e}", exc_info=True
        )
        return ""
...
def validate_xml_against_xsd(xml_file_path, xsd_file_path=None):
    """
    Valida un archivo XML contra un esquema XSD con protección XXE.
    ...
    Security:
        - Protección contra XXE (XML External Entity) attacks
        - Límite de tamaño de archivo (10 MB)
        - Parser sin acceso a red ni entidades externas
    """
    # Parser seguro que deshabilita entidades externas (protección XXE)
    parser_seguro = etree.XMLParser(
        resolve_entities=False,  # ⭐ Deshabilitar entidades externas
        no_network=True,  # ⭐ Sin acceso a red
        dtd_validation=False,  # ⭐ Sin DTD
        load_dtd=False,  # ⭐ No cargar DTD
    )
    ...
    try:
        ...
        # Validar tamaño del XML (prevenir XML Bomb - máximo 10 MB)
        xml_size = os.path.getsize(xml_file_path)
        if xml_size > 10 * 1024 * 1024:  # 10 MB
            return (
                "50",
                f"Error: XML excede tamaño máximo permitido (10MB). Tamaño: {xml_size / (1024*1024):.2f}MB",
            )
        ...
        if xsd_file_path is None:
            ...
            try:
                ...
                logger.error(f"Error cargando configuración XSD: {e}", exc_info=True)
                return (
                    "55",
                    "Error: No se pudo cargar la configuración de rutas de los XSD.",
                )
            ...
            try:
                ...
                logger.error(
                    f"Error detectando tipo de comprobante: {e}", exc_info=True
                )
                return (
                    "54",
                    "Error: No se pudo detectar el tipo de comprobante.",
                )
        ...
        # Cargar y parsear el esquema XSD con parser seguro
        xmlschema_doc = etree.parse(xsd_file_path, parser_seguro)
        xmlschema = etree.XMLSchema(xmlschema_doc)

        # Cargar y validar el documento XML con parser seguro
        xml_doc = etree.parse(xml_file_path, parser_seguro)
        ...
        logger.error(f"Error de sintaxis XML: {e}", exc_info=True)
        return ("50", f"Error de sintaxis XML: {str(e)}")

    except Exception as e:
        # Capturar cualquier excepción y devolverla como mensaje de error
        logger.error(f"Error inesperado validando XML: {e}", exc_info=True)
        return (
            "50",
            "Error inesperado durante la validación del XML. Consulte los logs para más detalles.",
        )
...
def UnlockCK():
    try:
        ...
        if not success:
            ...
            log_event(logger, "info", sanitize_for_log(glob.lastErrorText()))
...
def ObtenerSemilla(Origen=1, URLContribuyente=None):
    try:
        ...
        log_event(logger, "info", f"URL consultada: {sanitize_for_log(url)}")

        # Cliente HTTPX con soporte TLS y reconexión automática
        with httpx.Client(timeout=10, verify=True) as client:
            log_event(logger, "info", f"Conectando a: {sanitize_for_log(url)}")
        ...
        if respStatusCode >= 400:
            ...
            log_event(
                logger,
                "error",
                f"Response Header: {sanitize_for_log(str(response.headers))}",
            )
            log_event(
                logger, "error", f"Response Body: {sanitize_for_log(response.text)}"
            )
            return ""
...
def FirmarXML(RNCEmisor, Archivo_xml, xml_tag, CodigoSeguridadCF=None):
    ...
    while not FirmaCorrecta and intentos < max_intentos:
        ...
        if not success:
            log_event(logger, "info", sanitize_for_log(gen.LastErrorText))
            return "56", gen.LastErrorText, "", "", ""

        # Determinar la ruta destino del archivo firmado (con protección path traversal)
        try:
            nombre_archivo = os.path.splitext(os.path.basename(Archivo_xml))[0] + ".xml"

            if xml_tag == "SemillaModel":
                Archivo_xml_firmado = generar_ruta_segura(
                    GConfig.FEDGII.RutaSemilla, ["firmadas"], nombre_archivo
                )
            elif xml_tag == "ECF":
                Archivo_xml_firmado = generar_ruta_segura(
                    GConfig.FEDGII.RutaXML, ["firmadas"], nombre_archivo
                )
            elif xml_tag == "RFCE":
                Archivo_xml_firmado = generar_ruta_segura(
                    GConfig.FEDGII.RutaXML, ["firmadas", "resumen"], nombre_archivo
                )
            elif xml_tag == "ACECF":
                Archivo_xml_firmado = generar_ruta_segura(
            ...
            log_event(logger, "error", f"Path traversal detectado: {e}")
            return "56", f"Ruta inválida: {e}", "", "", ""

        if os.path.exists(Archivo_xml_firmado):
            generar_nombre_incremental(Archivo_xml_firmado)

        # Escribir archivo de forma atómica (previene race conditions)
        if not escribir_xml_atomico(sbXml.GetAsString(), Archivo_xml_firmado):
            error_msg = f"Error al escribir el archivo XML firmado: {os.path.basename(Archivo_xml_firmado)}"
        ...
        if not verifier.LoadSignatureSb(sbXml):
            log_event(logger, "info", sanitize_for_log(verifier.LastErrorText))
...
def ConsultaDirectorioServiciosListado(bearer_btoken):
    ...
    if success != True:
        ...
        log_event(logger, "error", sanitize_for_log(rest.LastErrorText))
        # sys.exit()
    ...
    if respStatusCode >= 400:
        ...
        log_event(logger, "error", sanitize_for_log(rest.ResponseHeader))
        log_event(logger, "error", "Response Body:")
        log_event(logger, "error", sanitize_for_log(sbResponseBody.GetAsString()))
        # sys.exit()
    ...
    log_event(logger, "info", sanitize_for_log(jsonResponse.Emit()))
...
def ConsultaDirectorioServiciosRNC(RNC, bearer_btoken):
    ...
    if success != True:
        log_event(logger, "info", sanitize_for_log(rest.LastErrorText))
        # sys.exit()
...
def GenerarXMLAprobacionComercial(cn1, RNCEmisor, eNCF):
    ...
    query = """
        SELECT * 
        FROM AprobacionComercial WITH (READUNCOMMITTED)
        WHERE eNCF = ? 
        AND RNCEmisor = ?
    """
    qAprobacionComercial = cn1.fetch_query(query, (eNCF, RNCEmisor))
...
def GenerarXML(cn1, RNCEmisor, eNCF):
    try:
        ...
        #   2. Los totales del encabezado coincidan con los totales generados por el ERP
        #
        # Debido a que algunos ERPs calculan o consolidan los totales de forma
        # diferida (por trigger, vista o proceso asíncrono), se realizan 3 intentos
        # con pequeñas pausas para evitar falsos negativos.
        #
        # REGLAS:
        #   ✔ Si después de 3 intentos no existe detalle → ERROR código 62
        #   ✔ Si después de 3 intentos totales no coinciden → ERROR código 61
        #   ✔ Si totales coinciden → continuar
        #   ✔ Si no existe vista vFETotales → se considera válido con detalle
        #
        # Ventaja: evita rechazos fiscales por XML incompletos o inconsistentes.
        # ============================================================================
        ...
        for intento in range(1, 3):
            ...
            # Se reintenta hasta el tercer intento.
            # Al tercer intento se devuelve código 62.
            # ------------------------------------------------------------------------
            ...
            if cn1.vista_existe("vFETotales"):
                ...
                if qTotales:
                    ...
                    if round(diferencia, 2) > 1:
                        ...
                        # Tercer intento → ERROR código 61
            ...
            # ------------------------------------------------------------------------
            # Si no existe vista de totales, se asume que el ERP no utiliza consolidación
            # por vista y se continúa con los totales contenido en el query de Encabezado.
            # ------------------------------------------------------------------------
            log_event(
                logger,
                "info",
        ...
        if (
            qEncabezadoFactura[0].TelefonoEmisor1 is not None
            and qEncabezadoFactura[0].TelefonoEmisor1.strip() != ""
            and validar_telefono_rd(
                (qEncabezadoFactura[0].TelefonoEmisor1.strip() or "")
            )
        ):
            xml.UpdateChildContent(
                "Encabezado|Emisor|TablaTelefonoEmisor|TelefonoEmisor[0]",
        ...
        if (
            ...
            and validar_telefono_rd(
                (qEncabezadoFactura[0].TelefonoEmisor2.strip() or "")
            )
        ):
            xml.UpdateChildContent(
        ...
        if (
            qEncabezadoFactura[0].TelefonoEmisor3 is not None
            and qEncabezadoFactura[0].TelefonoEmisor3.strip() != ""
            and validar_telefono_rd(
                (qEncabezadoFactura[0].TelefonoEmisor3.strip() or "")
            )
        ):
            xml.UpdateChildContent(
        ...
        # Dato correspondiente al correo electrónico del emisor.
        # Tamaño: 80
        # Tipo: ALFA NUM
        # Validación: Formato válido: Estructura de  correo electrónico  (xxxxx@xxx.xx)
        # Impresion Oblicatoria: No
        # OBLIGATORIEDAD:
        # No Obligatorio para todos lso comprobantes
        ...
        if qTotales:
            ...
            if (
                ...
                xml.UpdateChildContent(
                    "Encabezado|Totales|TotalITBIS1",
                    f"{Decimal
                            (qEncabezadoFactura[0].TotalITBIS1):.2f}",
                )
...
def GenerarXMLRFCE(cn1, RNCEmisor, eNCF):
    ...
    RNCEmisor = RNCEmisor.strip()
    eNCF = eNCF.strip()
    # Generar los datos necesarios para llenar lso XMLS de los Documentos Electronicos

    #  Consultar vFEEncabezado
    qEncabezadoFactura = cn1._get_encabezado(RNCEmisor.strip(), eNCF.strip())

    #  Consultar vTablaPago
    qTablaPago = cn1._get_tablapago(RNCEmisor.strip(), eNCF.strip())

    # Consultar Tabla impuestos Adicionales
    qTablaImpuestosAdicionales = cn1._get_tablaimpuestosadicionales(
        RNCEmisor.strip(), eNCF.strip()
    )
    ...
    # Nuevas Lienas con XML Nativo de Python
    xml = XmlNative()
    xml.Tag = "ECF"
    ...
    # Totales
    if qTotales:
        if qTotales[0].MontoGravadoTotal is not None:
            if qTotales[0].MontoGravadoTotal > 0 or qTotales[0].MontoTotal == 0:
                xml.UpdateChildContent(
                    "Encabezado|Totales|MontoGravadoTotal",
                    f"{Decimal(qTotales[0].MontoGravadoTotal):.2f}",
                )
            ...
            if (
                ...
                xml.UpdateChildContent(
                    "Encabezado|Totales|MontoGravadoI1",
                    f"{Decimal(qTotales[0].MontoGravadoI1):.2f}",  # 16%
                )
            if (
                qTotales[0].MontoGravadoI2 is not None
                and TipoECF not in [43, 44, 46, 47]
                ...
                xml.UpdateChildContent(
                    "Encabezado|Totales|MontoGravadoI2",
                    f"{Decimal(qTotales[0].MontoGravadoI2):.2f}",  # 18%
                )

            if (
                qTotales[0].MontoGravadoI3 is not None
                and TipoECF not in [43, 44, 46, 47]
                and qTotales[0].IndicadorMontoGravadoIEX == 1
                and qEncabezadoFactura[0].IndicadorNotaCredito == 0
            ):
                if float(qTotales[0].MontoGravadoI3) > 0:
                    xml.UpdateChildContent(
                        "Encabezado|Totales|MontoGravadoI3",
                        f"{Decimal(qTotales[0].MontoGravadoI3):.2f}",  # 0% (Exento)
                    )
        ...
        if (
            ...
            and qTotales[0].IndicadorMontoGravadoIE == 1
            and qEncabezadoFactura[0].IndicadorNotaCredito == 0
            and TipoECF not in [46]
        ):
            if float(qTotales[0].MontoExento) > 0:
                xml.UpdateChildContent(
                    "Encabezado|Totales|MontoExento",
                    f"{Decimal(qTotales[0].MontoExento):.2f}",
                )

        if qTotales[0].TotalITBIS is not None and TipoECF not in [43, 44, 47]:
            xml.UpdateChildContent(
                "Encabezado|Totales|TotalITBIS",
                f"{Decimal(qTotales[0].TotalITBIS):.2f}",
            )
        ...
        if (
            ...
            if int(qTotales[0].TotalITBIS1) > 0:
                xml.UpdateChildContent(
                    "Encabezado|Totales|TotalITBIS1",
                    f"{Decimal
                            (qTotales[0].TotalITBIS1):.2f}",
        ...
        if (
            ...
            if int(qTotales[0].TotalITBIS2) > 0:
                xml.UpdateChildContent(
                    "Encabezado|Totales|TotalITBIS2",
                    f"{Decimal(qTotales[0].TotalITBIS2):.2f}",
                )
        ...
        if (
            ...
            if int(qTotales[0].TotalITBIS3) > 0:
                xml.UpdateChildContent(
                    "Encabezado|Totales|TotalITBIS3",
                    f"{Decimal(qTotales[0].TotalITBIS3 or 0):.2f}",
                )
    else:
        if qEncabezadoFactura[0].MontoGravadoTotal is not None:
            if (
                qEncabezadoFactura[0].MontoGravadoTotal > 0
                or qEncabezadoFactura[0].MontoTotal == 0
            ):
                xml.UpdateChildContent(
                    "Encabezado|Totales|MontoGravadoTotal",
                    f"{Decimal(qEncabezadoFactura[0].MontoGravadoTotal):.2f}",
            ...
            if (
                ...
                xml.UpdateChildContent(
                    "Encabezado|Totales|MontoGravadoI1",
                    f"{Decimal(qEncabezadoFactura[0].MontoGravadoI1):.2f}",  # 16%
                )
            if (
                qEncabezadoFactura[0].MontoGravadoI2 is not None
                and TipoECF not in [43, 44, 46, 47]
                ...
                xml.UpdateChildContent(
                    "Encabezado|Totales|MontoGravadoI2",
                    f"{Decimal(qEncabezadoFactura[0].MontoGravadoI2):.2f}",  # 18%
                )

            if (
                qEncabezadoFactura[0].MontoGravadoI3 is not None
                and TipoECF not in [43, 44, 46, 47]
                and qEncabezadoFactura[0].IndicadorMontoGravadoIEX == 1
                and qEncabezadoFactura[0].IndicadorNotaCredito == 0
            ):
                if float(qEncabezadoFactura[0].MontoGravadoI3) > 0:
                    xml.UpdateChildContent(
                        "Encabezado|Totales|MontoGravadoI3",
                        f"{Decimal(qEncabezadoFactura[0].MontoGravadoI3):.2f}",  # 0% (Exento)
                    )
        ...
        if (
            ...
            if float(qEncabezadoFactura[0].MontoExento) > 0:
                xml.UpdateChildContent(
                    "Encabezado|Totales|MontoExento",
                    f"{Decimal(qEncabezadoFactura[0].MontoExento):.2f}",
                )

        if qEncabezadoFactura[0].TotalITBIS is not None and TipoECF not in [43, 44, 47]:
            xml.UpdateChildContent(
                "Encabezado|Totales|TotalITBIS",
                f"{Decimal(qEncabezadoFactura[0].TotalITBIS):.2f}",
            )
        ...
        if (
            ...
            if int(qEncabezadoFactura[0].TotalITBIS1) > 0:
                xml.UpdateChildContent(
                    "Encabezado|Totales|TotalITBIS1",
                    f"{Decimal
                            (qEncabezadoFactura[0].TotalITBIS1):.2f}",
        ...
        if (
            ...
            if int(qEncabezadoFactura[0].TotalITBIS2) > 0:
                xml.UpdateChildContent(
                    "Encabezado|Totales|TotalITBIS2",
                    f"{Decimal(qEncabezadoFactura[0].TotalITBIS2):.2f}",
                )
        ...
        if (
            ...
            if int(qEncabezadoFactura[0].TotalITBIS3) > 0:
                xml.UpdateChildContent(
                    "Encabezado|Totales|TotalITBIS3",
                    f"{Decimal(qEncabezadoFactura[0].TotalITBIS3 or 0):.2f}",
                )
    ...
    # 💾 Guardar el nuevo XML con el nombre original
    xml.SaveXml(NombreXML)
    log_event(logger, "info", f"Nuevo archivo guardado como: {NombreXML}")

    return True  # Realizado
...
def ConsultaEstadoECF(token, rncemisor, ENCF, rnccomprador=None, codigoseguridad=None):
    ...
    if respStatusCode >= 400:
        ...
        log_event(logger, "info", sanitize_for_log(rest.ResponseHeader))
        log_event(logger, "info", "Response Body:")
        log_event(logger, "info", sanitize_for_log(sbResponseBody.GetAsString()))
        # sys.exit()
...
def ConsultaResultadoRFCE(RncEmisor, ENCF, CodigoSeguridad, token):
    ...
    try:
        ...
        if response.status_code >= 400:
            ...
            log_event(logger, "error", "Response Header:")
            log_event(logger, "error", sanitize_for_log(str(response.headers)))
            log_event(logger, "error", "Response Body:")
            log_event(logger, "error", sanitize_for_log(response.text))
        ...
        log_event(logger, "info", sanitize_for_log(response.text))
        ...
        try:
            ...
            log_event(
                logger,
                "error",
                f"Contenido recibido: '{sanitize_for_log(response.text)}'",
            )
            return None
    ...
    for intento in range(max_retries):
        ...
        try:
            try:
                ...
                logger.debug(f"Error logging intento: {e}")
            ...
            try:
                ...
                logger.debug(f"Error logging respuesta HTTP: {e}")
            ...
            if status >= 400:
                ...
                try:
                    ...
                    logger.debug(f"Respuesta no es JSON válido: {e}")
...
def AnulacionNCF():
    ...
    if respStatusCode >= 400:
        ...
        log_event(logger, "info", sanitize_for_log(rest.ResponseHeader))
        log_event(logger, "info", "Response Body:")
        log_event(logger, "info", sanitize_for_log(strResponseBody))
        # sys.exit()
    ...
    while i < count_i:
        jsonResponse.I = i
        jsonResponse.StringOf("mensajes[i]")
        i = i + 1
...
def EnvioAprobacionComercial(xml_path, bearer_token):
    ...
    req.AddFileForUpload2("xml", xml_path, "text/xml")
    ...
    if http.LastMethodSuccess == False:
        log_event(logger, "info", sanitize_for_log(http.LastErrorText))
        # sys.exit()
    ...
    log_event(logger, "info", "Response Body:")
    log_event(logger, "info", sanitize_for_log(jResp.Emit()))
...
def EnvioRFCE(xml_path, bearer_token):
    ...
    try:
        ...
        if http.LastMethodSuccess == False:
            log_event(logger, "error", sanitize_for_log(http.LastErrorText))
            Intentos += 1
            if Intentos < Max:
...
def ObtennerToken(
    ...
    RNCEmisor,
    Origen=1,
    URLContribuyente=None,
    max_reintentos=3,
    espera_segundos=5,
    token_cache=None,
):
    """
    Obtiene token de autenticación para DGII con soporte de cache.

    Args:
        cn1: Conexión a BD
        RNCEmisor: RNC del emisor
        Origen: Origen del token (1=DGII, 2=Contribuyente)
        URLContribuyente: URL opcional del contribuyente
        max_reintentos: Máximo de reintentos
        espera_segundos: Segundos entre reintentos
        token_cache: Dict opcional para cachear tokens {RNC: (token, expiry_timestamp)}

    Returns:
        str: Token de autenticación
    """
    # 🚀 CACHE: Verificar cache en memoria primero (ultra rápido)
    if token_cache is not None and RNCEmisor in token_cache:
        cached_token, expiry_ts = token_cache[RNCEmisor]
        if time.time() < expiry_ts - 300:  # Margen de 5 min
            return cached_token

    # 1️⃣ Revisar BD para token vigente (CORREGIDO SQL Injection)
    query = """
        SELECT TOP 1 * 
        FROM FEToken 
        WHERE rnc=? 
        ...
        ORDER BY expedido DESC
    """
    qLocalToken = cn1.fetch_query(query, (RNCEmisor, GConfig.FEDGII.Ambiente))
    ...
    if qLocalToken:
        ...
        if isinstance(expira_dt, str):
            expira_dt = datetime.fromisoformat(expira_dt.replace("Z", "+00:00"))
        if isinstance(expedido_dt, str):
            expedido_dt = datetime.fromisoformat(expedido_dt.replace("Z", "+00:00"))

        # Asegurar que tienen timezone
        if expira_dt.tzinfo is None:
            expira_dt = expira_dt.replace(tzinfo=timezone.utc)
        if expedido_dt.tzinfo is None:
            expedido_dt = expedido_dt.replace(tzinfo=timezone.utc)
    ...
    for intento in range(1, max_reintentos + 1):
        try:
            ...
            ArchivoToken = os.path.join(
                GConfig.FEDGII.URLBase,
                GConfig.FEDGII.RutaToken,
                f'token{datetime.now().strftime("%Y%m%d_%H%M%S_%f")}.xml',
            )

            # Determinar URL según Origen
            if Origen == 2 and URLContribuyente:
                base_url = f"{URLContribuyente}/api/semilla/validacioncertificado"
            else:
                base_url = f"https://{GConfig.FEDGII.URLBase}{GConfig.FEDGII.URLAmbienteActivo}{GConfig.FEDGII.URLToken}"

            # CORREGIDO: Usar context manager para cerrar el archivo
            with open(SemillaFirmada, "rb") as xml_file:
                files = {
                    "xml": (
                        os.path.basename(SemillaFirmada),
                        xml_file,
                ...
                response = requests.post(
                    base_url, files=files, headers=headers, verify=True, timeout=30
                )
            ...
            expira = root.find(".//expira").text.replace("T", " ").replace("Z", "")
            expedido = root.find(".//expedido").text.replace("T", " ").replace("Z", "")
            expira_dt = datetime.fromisoformat(expira).replace(tzinfo=timezone.utc)
            expedido_dt = datetime.fromisoformat(expedido).replace(tzinfo=timezone.utc)
            ...
            log_event(
                logger,
                "error",
                f"Fallo en intento {intento}: {str(e)}: {traceback.format_exc()}",
            )
            if intento < max_reintentos:
                time.sleep(espera_segundos)
            else:
...
def EnvioDGII(cn1, row, token_cache=None):
    """
    Envía un comprobante fiscal electrónico a DGII con manejo inteligente de errores.

    Args:
        cn1: Conexión a base de datos
        row: Fila con datos del comprobante
        token_cache: Dict opcional para cachear tokens

    Returns:
        (EstadoFiscal, Mensaje): Estado resultante y mensaje descriptivo

    Raises:
        Exception: Solo para errores recuperables (conectividad, timeout, etc.)
    """
    ...
    try:
        ...
        # Validar RNC Emisor
        es_valido, msg = validar_rnc(row.RNCEmisor.strip())
        if not es_valido:
            mensaje_error = f"RNC Emisor inválido: {msg}"
            logger.error(mensaje_error)

            # Actualizar estado fiscal a error de validación (70)
            cn1.actualizar_estado_fiscal(
                row.Tabla.strip(),
                70,  # Error de validación
                mensaje_error,
                row.campo1.strip(),
                row.campo2.strip(),
                row.RNCEmisor.strip(),
            ...
            return 70, mensaje_error

        # Validar eNCF
        es_valido, msg = validar_encf(row.eNCF.strip(), int(row.TipoECF))
        if not es_valido:
            mensaje_error = f"eNCF inválido: {msg}"
            logger.error(mensaje_error)
        ...
        # Validar montos (si aplica)
        if hasattr(row, "MontoSubTotal") and hasattr(row, "MontoTotal"):
            es_valido, msg = validar_montos_factura(
                str(row.TipoECF),
                row.MontoSubTotal or Decimal("0"),
                row.MontoITBIS or Decimal("0"),
                row.MontoTotal or Decimal("0"),
                (
                    row.Descuento or Decimal("0")
                    if hasattr(row, "Descuento")
                    else Decimal("0")
                ),
                (
                    row.Propina or Decimal("0")
                    if hasattr(row, "Propina")
                    else Decimal("0")
                ),
                es_resumen=(int(row.TipoECF) == 32 and row.MontoTotal < 250000),
            )
            if not es_valido:
                mensaje_error = f"Montos inválidos: {msg}"
                logger.error(mensaje_error)
        ...
        # ======================================
        # VALIDACIONES BÁSICAS (MANTENER COMPATIBILIDAD)
        # ======================================
        ...
        # Revisar si esta enviado - OPTIMIZADO con query parametrizada
        query = f"""
            SELECT 1 
            FROM {row.Tabla.strip()} WITH (READUNCOMMITTED)
            WHERE {row.campo1.strip()} = ? 
            AND {row.campo2.strip()} = ? 
            AND enviado = 1
        """
        Enviado = cn1.fetch_query(query, (row.RNCEmisor.strip(), row.eNCF.strip()))
            ...
            if int(row.EstadoFiscal) == 3:
                ...
                if (int(row.TipoECF) == 32) and (row.MontoTotal < 250000):
                    codigo, estado, Mensaje = EnvioRFCE(
                        NombreXML,
                        ObtennerToken(
                            cn1, row.RNCEmisor.strip(), token_cache=token_cache
                        ),
                    )

                    # Detectar error de conexión en EnvioRFCE
                    if codigo == 70 and "Conexión" in str(Mensaje):
                        raise ConnectionError(
                            f"Error de conexión al enviar RFCE: {Mensaje}"
                        )
                    ...
                    if EstadoFiscal == 99 or EstadoFiscal == 6:
                        ...
                        # Si ConsultaResultadoRFCE retornó None (error de conexión)
                        if datostmp is None:
                            raise ConnectionError(
                                "Error de conexión al consultar resultado RFCE"
                            )
                    ...
                    # Si Enviar_DFE retornó error de conexión, no continuar
                    if trackId == 70 and "Conexión" in str(Mensaje):
                        raise ConnectionError(
                            f"Error de conexión al enviar DFE: {Mensaje}"
                        )
                    ...
                    # Detectar errores de conexión en la consulta
                    if (
                        "error de conexión" in estado.lower()
                        or "error de cone" in estado.lower()
                    ):
                        raise ConnectionError(
                            f"Error de conexión al consultar trackId: {estado}"
                        )
                    ...
                    if EstadoFiscal == 99 or EstadoFiscal == 6:
                        ...
                        if datostmp:
                            datos = json.loads(datostmp)
                            # Verificar si el resultado indica error de conexión
                            if (
                                datos.get("codigo") == 99
                                and "error" in datos.get("estado", "").lower()
                            ):
                                raise ConnectionError(
                                    f"Error de conexión al consultar resultado ECF: {datos.get('estado')}"
                                )
                            m = datos["mensajes"]
                        else:
                            raise ConnectionError(
                                "Error de conexión al consultar resultado ECF (respuesta vacía)"
                            )
        ...
        # Clasificar el error para determinar si es recuperable
        ...
        logger.error(
            f"Error clasificado como {error_type.value}: {e_str}:"
            f"{traceback.extract_tb(sys.exc_info()[2])}"
        )

        # Errores recuperables: NO actualizar estado fiscal, relanzar excepción
        if error_type in [
            ErrorType.NETWORK,
            ErrorType.TIMEOUT,
            ...
            log_event(
                logger,
                "warning",
                f"Error recuperable detectado para NCF:{row.eNCF.strip()}. "
                f"NO se actualiza estado fiscal. Se reintentará.",
            )
            # Relanzar para que el loop principal maneje el reintento
            raise

        # Errores NO recuperables: actualizar estado fiscal
        Mensaje = e_str
        ...
        log_event(
            logger,
            "error",
            f"XML del NCF:{row.eNCF.strip()} del Emisor {row.RNCEmisor.strip() or ''}-{row.RazonSocialEmisor.strip()} y el comprador:{row.RNCEmisor.strip()}-{row.RazonSocialComprador.strip()} Generó el siguiente Mensaje: {e_str}",
        )
        return 70, e_str
...
def GenerarYFirmar(cn1, row):
    try:
        ...
        es_valido, msg = validar_rnc(row.RNCEmisor.strip())
        if not es_valido:
            raise ValueError(f"RNC del Emisor inválido: {msg}")

        # Validar eNCF
        if row.eNCF.strip() is None:
            raise TypeError("El NCF está vacio.")

        es_valido, msg = validar_encf(row.eNCF.strip())
        if not es_valido:
            raise ValueError(f"eNCF inválido: {msg}")
        # Generar XML
        EstadoFiscal = "00"
        ...
        if (
            ...
            # Sanitizar log (ocultar datos personales completos)
            log_event(
                logger,
                "info",
                f"NCF generado: {row.eNCF.strip()} - Emisor RNC: {row.RNCEmisor.strip()[:7]}***",
            )
            ...
            # Sanitizar log (mostrar solo nombre de archivo, no path completo)
            log_event(
                logger, "info", f"Firmando archivo: {os.path.basename(NombreXML)}"
            )
            ...
            log_event(logger, "info", f"Archivo firmado: {os.path.basename(NombreXML)}")
            if (int(row.TipoECF) == 32) and (row.MontoTotal < 250000):
                Ruta = "generadas/resumen/"
                NombreXMLR = f"{GConfig.FEDGII.RutaXML}{Ruta}{row.RNCEmisor.strip()+row.eNCF.strip()}.xml"
                log_event(
                    logger, "info", f"Firmando resumen: {os.path.basename(NombreXMLR)}"
                )
                ...
                if codigo == 56:
                    ...
                    return codigo, mensaje
                log_event(
                    logger, "info", f"Resumen firmado: {os.path.basename(NombreXMLR)}"
                )
            ...
            log_event(
                logger,
                "info",
                f"Validando archivo: {os.path.basename(NombreXMLFirmado)}",
            )
            ...
            if codigov == "03":
                ...
                # Sanitizar log para proteger datos personales
                log_event(
                    logger,
                    "info",
                    f"NCF {row.eNCF.strip()} firmado y validado exitosamente - Emisor RNC: {row.RNCEmisor.strip()[:7]}***",
                )
                return "03", mensajev
                ...
                # Sanitizar log y mensaje de error
                log_event(
                    logger,
                    "info",
                    f"NCF {row.eNCF.strip()} validación falló - Código: {codigov} - Mensaje: {sanitize_for_log(ResultadoFiscalT)}",
                )
        ...
        # Error de validación - no recuperable
        logger.error(f"Error de validación en GenerarYFirmar: {e}", exc_info=True)
        mensaje_sanitizado = sanitize_for_log(str(e))
        return "70", f"Validación falló: {mensaje_sanitizado}"

    except FileNotFoundError as e:
        # Error de archivo - no recuperable
        logger.error(f"Archivo no encontrado en GenerarYFirmar: {e}", exc_info=True)
        return "70", f"Archivo faltante: {str(e)}"

    except Exception as e:
        # Error inesperado
        logger.error(
            "Error inesperado en GenerarYFirmar",
            exc_info=True,
            extra={
                "rnc_emisor": (
                    row.RNCEmisor.strip() if hasattr(row, "RNCEmisor") else "N/A"
                ),
                "encf": row.eNCF.strip() if hasattr(row, "eNCF") else "N/A",
