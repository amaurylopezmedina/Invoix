...
def ensure_sqlite_vistas():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS Vistas (
            ...
            descripcion TEXT,
            codigo TEXT,
            UNIQUE(esquema, nombre)
        )
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS VistaHistorial (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            esquema TEXT,
            nombre TEXT,
            version INTEGER,
...
ensure_sqlite_vistas()

class GestorVistas(tk.Tk, FindReplaceMixin):
    def __init__(self):
        super().__init__()
        self.title("Gestor de Vistas SQL (SQL Server + SQLite) — v5.0")
    ...
    def _build_ui(self):
        ...
        left = ttk.LabelFrame(top, text="Vistas guardadas (SQLite)", padding=8)
        left.pack(side="left", fill="both", expand=True, padx=(0,6))

        self.tree_local = ttk.Treeview(left, columns=("id","esquema","nombre","descripcion"), show="headings", height=14)
        for c,w in [("id",70),("esquema",180),("nombre",420),("descripcion",560)]:
        ...
        right = ttk.LabelFrame(top, text="Vistas en SQL Server", padding=8)
        ...
        ttk.Button(filt, text="📦 Importar TODAS", command=self._import_all_to_sqlite).pack(side="left", padx=6)
        ...
        ttk.Button(btns, text="➕ Nueva", command=self._new_view).pack(side="left", padx=6)
        ...
        ttk.Button(btns, text="☁️ Sincronizar SQLite → SQL Server (todas)", command=self._sync_sqlite_to_sqlserver).pack(side="left", padx=12)
    ...
    def _load_list_sqlite(self):
        ...
        cur = self.sqlite_conn.cursor(); cur.execute("SELECT id,esquema,nombre,descripcion FROM Vistas ORDER BY esquema, nombre")
    ...
    def _reload_server_list(self):
        ...
        try:
            ...
            messagebox.showerror("Error", f"No se pudo leer la lista de vistas:\n{e}")
            return
    ...
    def _on_select_local(self, event=None):
        ...
        if not sel: return
        item = self.tree_local.item(sel[0]); vista_id = item["values"][0]
        cur = self.sqlite_conn.cursor(); cur.execute("SELECT codigo FROM Vistas WHERE id=?", (vista_id,))
        row = cur.fetchone()
    ...
    def _load_selected_server_to_editor(self):
        ...
        if not sel: messagebox.showwarning("Atención", "Seleccione una vista del servidor."); return
        ...
        try:
            with engine.connect() as conn:
                rs = conn.execute(text("""
                    ...
                    WHERE so.type = 'V' AND so.name = :n AND s.name = :s
                """), {"n": name, "s": schema})
                row = rs.fetchone()
                if row and row[0]:
                    self.text_sql.delete("1.0", "end"); self.text_sql.insert("1.0", row[0])
                    messagebox.showinfo("Importado", f"Vista '{schema}.{name}' cargada en el editor.")
                else:
    ...
    def _new_view(self):
        ...
        name = simpledialog.askstring("Nombre", "Nombre de la vista:") or "NuevaVista"
        plantilla = f"CREATE OR ALTER VIEW {quotename(schema)}.{quotename(name)}\nAS\nSELECT TOP 100 * FROM sys.objects;\n"
    ...
    def _save_sqlite(self):
        ...
        esquema = simpledialog.askstring("Esquema", "Esquema de la vista (ej. dbo):") or "dbo"
        nombre = simpledialog.askstring("Nombre", "Nombre de la vista:")
        if not nombre: return
        descripcion = simpledialog.askstring("Descripción", "Descripción (opcional):") or ""
        try:
            cur = self.sqlite_conn.cursor()
            cur.execute("""
                INSERT OR REPLACE INTO Vistas (id, esquema, nombre, descripcion, codigo)
                VALUES ((SELECT id FROM Vistas WHERE esquema=? AND nombre=?), ?, ?, ?, ?)
            """, (esquema, nombre, esquema, nombre, descripcion, codigo))
            cur.execute("SELECT COALESCE(MAX(version),0) FROM VistaHistorial WHERE esquema=? AND nombre=?", (esquema, nombre))
            next_ver = (cur.fetchone()[0] or 0) + 1
            cur.execute("INSERT INTO VistaHistorial (esquema, nombre, version, fecha, codigo) VALUES (?,?,?,?,?)",
                        (esquema, nombre, next_ver, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), codigo))
            self.sqlite_conn.commit()
            log_event("VIEW","SAVE_SQLITE", f"{esquema}.{nombre}", "Guardada en SQLite")
            messagebox.showinfo("Guardado", f"Vista '{esquema}.{nombre}' guardada (v{next_ver}).")
            self._load_list_sqlite()
        except Exception as e:
    ...
    def _backup_current_server(self, schema, name):
        ...
        try:
            with engine.connect() as conn:
                ...
                row = rs.fetchone()
                if row and row[0]:
                    path = backup_server_code("vistas", f"{schema}.{name}", row[0])
                    return path
    ...
    def _apply_sqlserver(self):
        ...
        try:
            ...
            log_event("VIEW","APPLY_SQLSERVER","", "Aplicada en servidor")
            messagebox.showinfo("Éxito", "Vista creada/actualizada en SQL Server.")
            self._reload_server_list()
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo aplicar en SQL Server:\n{e}")

    def _drop_sqlserver(self):
        sel = self.tree_srv.selection()
        if not sel: messagebox.showwarning("Atención", "Seleccione una vista para eliminar."); return
        item = self.tree_srv.item(sel[0]); schema, name = item["values"][0], item["values"][1]
...
IF OBJECT_ID(N'{full}', N'V') IS NOT NULL
    DROP VIEW {quotename(schema)}.{quotename(name)};
""" )
            log_event("VIEW","DROP_SQLSERVER",f"{schema}.{name}","Eliminada")
            messagebox.showinfo("Eliminada", f"{schema}.{name} eliminada (si existía).")
            self._reload_server_list()
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo eliminar en SQL Server:\n{e}")
    ...
    def _diff_selected(self):
        ...
        cur = self.sqlite_conn.cursor(); cur.execute("SELECT codigo FROM Vistas WHERE esquema=? AND nombre=?", (schema, name))
    ...
    def _import_selected_to_sqlite(self):
        sel = self.tree_srv.selection()
        if not sel: messagebox.showwarning("Atención", "Seleccione una vista."); return
        item = self.tree_srv.item(sel[0]); schema, name = item["values"][0], item["values"][1]
        engine = make_engine(None)
        try:
            with engine.connect() as conn:
        ...
        try:
            cur.execute("""
                INSERT OR REPLACE INTO Vistas (id, esquema, nombre, descripcion, codigo)
                VALUES ((SELECT id FROM Vistas WHERE esquema=? AND nombre=?), ?, ?, ?, ?)
            """, (schema, name, schema, name, "", codigo))
            cur.execute("SELECT COALESCE(MAX(version),0) FROM VistaHistorial WHERE esquema=? AND nombre=?", (schema, name))
            next_ver = (cur.fetchone()[0] or 0) + 1
            cur.execute("INSERT INTO VistaHistorial (esquema, nombre, version, fecha, codigo) VALUES (?,?,?,?,?)",
                        (schema, name, next_ver, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), codigo))
            self.sqlite_conn.commit()
            log_event("VIEW","IMPORT_ONE",f"{schema}.{name}","Importada a SQLite")
            messagebox.showinfo("Importado", f"{schema}.{name} importada (v{next_ver}).")
            self._load_list_sqlite()
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo guardar en SQLite:\n{e}")

    def _import_all_to_sqlite(self):
        if not messagebox.askyesno("Confirmar", "¿Importar TODAS las vistas a SQLite?"): return
        engine = make_engine(None)
        total=ok=fail=0
        try:
            with engine.connect() as conn:
                rs = conn.execute(text('''
                    ...
                    ORDER BY s.name, v.name
                '''))
                objs = rs.fetchall(); total = len(objs)
                cur = self.sqlite_conn.cursor()
                for schema, name in objs:
                    try:
                        ...
                        cur.execute("""
                            INSERT OR REPLACE INTO Vistas (id, esquema, nombre, descripcion, codigo)
                            VALUES ((SELECT id FROM Vistas WHERE esquema=? AND nombre=?), ?, ?, ?, ?)
                        """, (schema, name, schema, name, "", code))
                        cur.execute("SELECT COALESCE(MAX(version),0) FROM VistaHistorial WHERE esquema=? AND nombre=?", (schema, name))
                        next_ver = (cur.fetchone()[0] or 0) + 1
                        cur.execute("INSERT INTO VistaHistorial (esquema, nombre, version, fecha, codigo) VALUES (?,?,?,?,?)",
                                    (schema, name, next_ver, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), code))
                        ok += 1
                    except Exception:
                        fail += 1
                self.sqlite_conn.commit()
        ...
        log_event("VIEW","IMPORT_ALL","", f"OK={ok} FAIL={fail} TOTAL={total}")
        messagebox.showinfo("Importación", f"Total: {total}\nOK: {ok}\nFallidos: {fail}")

    def _simulate_sync_sqlite_to_sqlserver(self):
        if not messagebox.askyesno("Simulación", "Validar sintaxis (SET NOEXEC ON) de TODAS las vistas. ¿Continuar?"): return
        ...
        ruta = os.path.join(DATA_DIR, f"dryrun_vistas_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
        engine = make_engine(None)
        cur = self.sqlite_conn.cursor(); cur.execute("SELECT esquema, nombre, codigo FROM Vistas ORDER BY esquema, nombre")
        rows = cur.fetchall()
        total = len(rows); ok = 0; fail = 0; lines = []
        try:
            with engine.connect() as conn:
                for esquema, nombre, codigo in rows:
                    if not codigo: continue
                    sql_clean = strip_go(codigo)
                    try:
                        conn.exec_driver_sql("SET NOEXEC ON; " + sql_clean + "; SET NOEXEC OFF;")
                        ok += 1; lines.append(f"[OK] {esquema}.{nombre}")
                    except Exception as ex:
                        fail += 1
                        preview = "\n".join(sql_clean.splitlines()[:n_preview])
                        lines.append(f"[ERROR] {esquema}.{nombre} :: {ex}\n--- Preview ---\n{preview}\n--- Fin ---")
        ...
        log_event("VIEW","DRYRUN","", f"OK={ok} FAIL={fail} TOTAL={total} REP={ruta}")
        messagebox.showinfo("Simulación", f"OK:{ok} FAIL:{fail} TOTAL:{total}\nReporte:\n{ruta}")

    def _sync_sqlite_to_sqlserver(self):
        if not messagebox.askyesno("Sincronizar", "Aplicar TODAS las vistas de SQLite en SQL Server (ejecutará scripts). ¿Continuar?"):
            return
        engine = make_engine(None)
        cur = self.sqlite_conn.cursor(); cur.execute("SELECT esquema, nombre, codigo FROM Vistas ORDER BY esquema, nombre")
        ...
        log_event("VIEW","SYNC_APPLY","", f"OK={ok} FAIL={fail} TOTAL={len(rows)}")
        messagebox.showinfo("Sincronización", f"Aplicadas OK:{ok} — Fallidas:{fail} — Total:{len(rows)}")

if __name__ == "__main__":
    GestorVistas().mainloop()
