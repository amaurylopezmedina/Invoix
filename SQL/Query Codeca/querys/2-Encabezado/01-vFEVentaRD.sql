...
SELECT
 ...
 CAST('' AS CHAR(1)) AS RazonModificacion,
  ...
  '' AS ModificadoPor,
  tr.cajero as Cajero,
  COALESCE(trim(e.nota), '')+'|'+COALESCE(trim(TR.OBSERVA1), '') +'|'+COALESCE(trim(TR.OBSERVA3), '') +'|'+COALESCE(trim(nc.nota), '')
  AS NotaPermanente,  
  tr.Descrip1 as NotaPago,
  '' as NotaAntesDeProductos,
  pccreado as EquipoImpresion
FROM
 ...
 LEFT OUTER JOIN NotaCaja as nc  WITH (NOLOCK) on nc.caja = SUBSTRING(tr.numero, 1, 1)  
