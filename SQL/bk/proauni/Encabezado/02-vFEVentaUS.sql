...
--Facturas a credito y contado en dolares
WITH
 e AS (
    SELECT top 1
      itbisenprecio,
...
SELECT
  ...
  tr.vence AS FechaLimitePago,
    (
    CASE
      WHEN tr.tipo = '79' THEN 0
      WHEN tr.tipo = '78' THEN tr.dia
    END
  ) AS TerminoPagoN,
    ...
    CASE
      WHEN tr.tipo = '79'
    THEN ''
      WHEN tr.tipo = '78' THEN CAST(tr.dia AS char (3)) + ' DIAS'
    END
  ) AS  TerminoPago,
  ...
  (
    CASE
      WHEN tr.tipo = '79' THEN 1
      WHEN tr.tipo = '78' THEN 2
    END
  ) AS TipoPago,
  (
    CASE
      WHEN tr.tipo = '79' THEN 'CONTADO'
      WHEN tr.tipo = '78' THEN 'CREDITO'
    END
  ) AS TipoPagoL,
  ...
  (
    CASE
      WHEN ISNULL (tr.rnc, '') = '' THEN REPLACE(c.rnc, '-', '')
      ELSE REPLACE(tr.rnc, '-', '')
    END
  ...
  ((tr.Monto-tr.itbis)* COALESCE(tr.tasa, 1.00)) as  MontoGravadoTotal,
  ((tr.grava-tr.itbis)* COALESCE(tr.tasa, 1.00)) as MontoGravadoI1, --18
 ...
 'PESOS DOMINICANO' AS TipoMonedaL,
  ...
  0 AS NumeroDocumentoNCFModificado,
 ...
 CAST('' AS CHAR(1)) AS RazonModificacion,
    ...
    CASE
        ...
        ELSE 
            CONCAT(
                ...
                CASE WHEN LEN(TRIM(AI.ruta)) > 0 THEN CONCAT(TRIM(AI.ruta), '/') ELSE '' END,
                'ConsultaTimbre?RncEmisor=', TRIM(TR.rncemisor),
                '&RncComprador=', TRIM(TR.rnc),  --El origen del rnc cambia en cada cliente y base de datos
                '&ENCF=', TRIM(TR.ncf),
                '&FechaEmision=', dbo.FNFechaDMY(TR.fecha),
                '&MontoTotal=', TR.monto,
                '&FechaFirma=', REPLACE(TRIM(TR.FechaFirma), ' ', '%20'),
  ...
  (
    CASE
      ...
      WHEN COALESCE(tr.observa, '') <> '' THEN tr.observa
      ELSE ''
    END
  ) AS NotaPermanente,
