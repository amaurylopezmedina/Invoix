...
SELECT
  ...
  DATEADD(DAY, 30, cxc.fecha)  AS FechaLimitePago,
  ...
  */
  ...
  REPLACE(cl.rnc1, '-', '') AS RNCComprador, --El origen del rnc cambia en cada cliente y base de datos
  ...
  cxc.Monto as  MontoGravadoTotal,
  cxc.grava as MontoGravadoI1, --18
  null as MontoGravadoI2, -- 16
  cxc.nograva as MontoGravadoI3,  -- Excento
  cxc.nograva as MontoExento,
  CASE
    WHEN cxc.grava <> 0 THEN 18 ELSE NULL
  END
  AS ITBIS1,
  NULL AS ITBIS2,
  CASE
    WHEN cxc.nograva <> 0 THEN 0 ELSE NULL
  END
  ITBIS3,
  cxc.impuesto as   TotalITBIS,
  cxc.impuesto TotalITBIS1,
  null as TotalITBIS2,
  0 as TotalITBIS3,
    CASE
    WHEN cxc.grava <> 0 THEN 1 ELSE 0
  END
  AS IndicadorMontoGravadoI18,
  0 AS IndicadorMontoGravadoI16,
  CASE
    WHEN cxc.nograva <> 0 THEN 1 ELSE 0
  END
  ...
  (
    CASE
      WHEN COALESCE(cxc.tasa, 1) =1 THEN 0
      ELSE cxc.monto
    END
  ...
  REPLACE(cxc.RNCEmisor, '-', '') AS RNCOtroContribuyente,
    ...
    CASE
        ...
        ELSE 
            CONCAT(
                ...
                CASE WHEN LEN(TRIM(AI.ruta)) > 0 THEN CONCAT(TRIM(AI.ruta), '/') ELSE '' END,
                'ConsultaTimbre?RncEmisor=', TRIM(cxc.rncemisor),
                '&RncComprador=', TRIM(cl.rnc1), --El origen del rnc cambia en cada cliente y base de datos
                '&ENCF=', TRIM(cxc.ncf),
                '&FechaEmision=', dbo.FNFechaDMY(cxc.fecha),
                '&MontoTotal=', cxc.monto,
                '&FechaFirma=', REPLACE(TRIM(cxc.FechaFirma), ' ', '%20'),
...
WITH
  ...
  LEFT JOIN TRANSA01 TR1 ON TRIM(TR1.documento) = TRIM(CXC.documento1) AND TR1.tipo = '04'
