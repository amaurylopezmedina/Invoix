...
SELECT
  ...
  (tr.Monto* COALESCE(tr.tasa, 1.00)) as  MontoGravadoTotal,
  (tr.grava* COALESCE(tr.tasa, 1.00)) as MontoGravadoI1, --18
  null as MontoGravadoI2, -- 16
  (tr.nograva* COALESCE(tr.tasa, 1.00)) as MontoGravadoI3,  -- Excento
  (tr.nograva* COALESCE(tr.tasa, 1.00)) as MontoExento,
  CASE
  ...
  END
  ITBIS3,
  (tr.itbis * COALESCE(tr.tasa, 1.00)) as   TotalITBIS,
  (tr.itbis* COALESCE(tr.tasa, 1.00)) TotalITBIS1,
  null as TotalITBIS2,
  0 as TotalITBIS3,
    CASE
    ...
    CASE
        ...
        ELSE 
            CONCAT(
                ...
                CASE WHEN LEN(TRIM(AI.ruta)) > 0 THEN CONCAT(TRIM(AI.ruta), '/') ELSE '' END,
                'ConsultaTimbre?RncEmisor=', TRIM(TR.rncemisor),
                '&RncComprador=', TRIM(TR.cedula),  --El origen del rnc cambia en cada cliente y base de datos
                '&ENCF=', TRIM(TR.ncf),
                '&FechaEmision=', dbo.FNFechaDMY(TR.fecha),
                '&MontoTotal=', TR.monto,
                '&FechaFirma=', REPLACE(TRIM(TR.FechaFirma), ' ', '%20'),
