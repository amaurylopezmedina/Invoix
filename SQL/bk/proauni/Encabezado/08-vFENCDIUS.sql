...
CREATE OR ALTER VIEW vFENCDIUS AS 
WITH
  e AS (
    SELECT
      itbisenprecio,
...
SELECT
  ...
  '' AS NombreVendedor,
  (cxc.monto * cxc.tasa) AS MontoTotal,
  NULL AS MontoNoFacturable,
  NULL AS MontoPeriodo,
  NULL AS SaldoAnterior,
  NULL AS MontoAvancePago,
  (cxc.monto * cxc.tasa) AS MontoPago,
  (cxc.monto * cxc.tasa) AS ValorPagar,
  ...
  cxc.EstadoFiscal,
    CASE
        WHEN SUBSTRING(cxc.ncf1, 2, 2) = '32' AND cxc.monto * cxc.tasa < 250000 
            THEN CONCAT(
...
FROM
  ...
  LEFT OUTER JOIN dbo.sis_TipoNCF AS tn WITH (NOLOCK) ON tn.Codigo = SUBSTRING(cxc.ncf1, 2, 2)
