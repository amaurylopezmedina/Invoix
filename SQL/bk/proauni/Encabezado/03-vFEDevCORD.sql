...
SELECT
 ...
 'CONTADO' AS TipoPagoL,
  ...
  (
    CASE
      WHEN ISNULL (tr.rnc, '') = '' THEN REPLACE(c.rnc /*collate SQL_Latin1_General_CP1_CI_AS*/, '-', '')
      ELSE REPLACE(tr.rnc, '-', '')
    END
...
FROM
  ...
  LEFT OUTER JOIN dbo.empresa AS e WITH (NOLOCK) ON tr.RNCEmisor = e.rnc /*collate SQL_Latin1_General_CP1_CI_AS*/
  LEFT OUTER JOIN dbo.sis_TipoNCF AS tn WITH (NOLOCK) ON tn.Codigo = SUBSTRING(tr.ncf, 2, 2) /*collate SQL_Latin1_General_CP1_CI_AS*/
  LEFT OUTER JOIN dbo.Transa01 AS tr1 WITH (nolock) ON tr1.numero = tr.documento and tr1.tipo = '03'
  CROSS JOIN AmbienteInfo AI
WHERE
  (tr.tipo IN ('17'))
  AND  COALESCE(tr.tasa, 1.00)= 1
