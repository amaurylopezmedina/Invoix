...
SELECT
  ...
  /*
      ...
      MAX(
        CASE
          WHEN td.itbis = 0 or cxc2.impuesto = 0 THEN 1
          ELSE 0
        END
      ) AS IndicadorMontoGravadoI0,
  */
  ...
  COALESCE(td_1.TotalITBISOtraMoneda,0) as  TotalITBISOtraMoneda,
  td_1.TotalITBIS1OtraMoneda,
  td_1.TotalITBIS2OtraMoneda,
  COALESCE(td_1.TotalITBIS3OtraMoneda,0) as TotalITBIS3OtraMoneda,
  NULL AS MontoImpuestoAdicionalOtraMoneda,
  (
    CASE
      WHEN COALESCE(cxc2.tasa, 1) = 1 THEN 0
      ELSE cxc2.monto
    END
...
FROM
  ...
  LEFT OUTER JOIN td_1 ON td_1.numero = cxc2.documento
  AND td_1.tipo = '05'
  AND cxc2.tipomovi = '03'
WHERE
