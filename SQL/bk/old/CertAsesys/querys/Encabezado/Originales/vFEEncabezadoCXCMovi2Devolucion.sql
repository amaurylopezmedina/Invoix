CREATE OR ALTER VIEW  vFEEncabezadoCXCMovi2Devolucion AS 
WITH

  td_1 AS (
    SELECT
      MAX(td.numero) AS numero,
      MAX(td.fecha) AS fecha,
      MAX(td.tipo) AS tipo,
      MAX(cxc2.RNCEmisor) AS RNCEmisor,
      MAX(cxc2.ncf) AS ncf,
      MAX(COALESCE(cxc2.tasa, 1.00)) AS tasa,
      MAX(e.itbisenprecio) AS IndicadorMontoGravado,
      SUM(
        CASE
          WHEN e.itbisenprecio = 0 THEN CASE
            WHEN td.itbis <> 0 THEN CASE
              WHEN COALESCE(cxc2.tasa, 1.00) = 1 THEN td.monto1 * 1
              ELSE td.monto1 * cxc2.tasa
            END
            ELSE 0.00
          END
          WHEN e.itbisenprecio = 1 THEN CASE
            WHEN td.itbis <> 0 THEN CASE
              WHEN COALESCE(cxc2.tasa, 1.00) = 1 THEN td.monto1 * 1
              ELSE td.monto1 * cxc2.tasa
            END
            ELSE 0.00
          END - CASE
            WHEN COALESCE(cxc2.tasa, 1.00) = 1 THEN td.montoitbis * 1
            ELSE td.montoitbis * cxc2.tasa
          END
        END
      ...
      SUM(
        CASE
          ...
          AND td.itbis = 18 THEN CASE
            WHEN td.itbis = 18 THEN CASE
              WHEN COALESCE(cxc2.tasa, 1.00) = 1 THEN td.monto1 * 1
              ELSE td.monto1 * cxc2.tasa
            END
            ELSE 0.00
      ...
      SUM(
        CASE
          WHEN e.itbisenprecio = 0
          AND td.itbis = 16 THEN CASE
            WHEN td.itbis = 16 THEN CASE
              WHEN COALESCE(cxc2.tasa, 1.00) = 1 THEN td.monto1 * 1
              ELSE td.monto1 * cxc2.tasa
            END
            ELSE 0.00
      ...
      SUM(
        CASE
          WHEN td.itbis = 0 THEN CASE
            WHEN COALESCE(cxc2.tasa, 1.00) = 1 THEN td.monto1 * COALESCE(cxc2.tasa, 1.00)
            ELSE td.monto1 * cxc2.tasa
          END
          ELSE NULL
        END
      ...
      SUM(
        CASE
          WHEN COALESCE(cxc2.tasa, 1) = 1 THEN td.montoitbis * COALESCE(cxc2.tasa, 1)
          ELSE td.montoitbis * cxc2.tasa
        END
      ) AS TotalITBIS,
      SUM(
        CASE
          WHEN td.itbis = 18 THEN td.montoitbis * COALESCE(cxc2.tasa, 1)
          ELSE NULL
        END
      ) AS TotalITBIS1,
      SUM(
        CASE
          WHEN td.itbis = 16 THEN td.montoitbis * COALESCE(cxc2.tasa, 1)
          ELSE NULL
        END
      ) AS TotalITBIS2,
      SUM(
      ...
      SUM(
        CASE
          WHEN td.itbis <> 0 THEN CASE
            WHEN COALESCE(cxc2.tasa, 1.00) = 1 THEN 1
            ELSE td.monto1
          END
          ELSE 0.00
        END
      ...
      SUM(
        CASE
          ...
          ELSE 0.00
        END
      ) AS MontoGravado2OtraMoneda,
      SUM(
        CASE
          WHEN td.itbis = 0 THEN 0
          ELSE 0
        END
      ) AS MontoGravado3OtraMoneda,
      SUM(
        CASE
          WHEN td.itbis = 0 THEN CASE
            WHEN COALESCE(cxc2.tasa, 0.00) = 0 THEN 0
            ELSE td.monto1
          END
          ELSE 0.00
        END
    ...
    WITH
      (NOLOCK)
      LEFT OUTER JOIN dbo.cxcmovi2 AS cxc2 ON cxc2.documento = td.numero
      AND td.tipo = '05'
      AND cxc2.tipomovi = '03'
      LEFT OUTER JOIN dbo.empresa AS e ON e.rnc = cxc2.RNCEmisor
    WHERE
      (cxc2.tipomovi IN ('03', '02'))
...
SELECT
  ...
  */
    (
	CASE
      WHEN SUBSTRING(cxc2.ncf, 2, 2) = '34' AND DATEDIFF(DAY, tr1.fecha, cxc2.fecha) >30 THEN 1
	  ELSE 0
    END
  ) AS IndicadorNotaCredito,
  ...
  (
    CASE
      ...
      WHEN cxc2.tipomovi = '03'
      OR cxc2.tipomovi = '02' THEN 'CREDITO'
    END
  ) AS TipoPagoL,
  ...
  td_1.TotalITBIS2,
  td_1.TotalITBIS3,
  td_1.IndicadorMontoGravadoI18,
  td_1.IndicadorMontoGravadoI16,
  td_1.IndicadorMontoGravadoI0,
  ...
  td_1.MontoGravado1OtraMoneda,
  td_1.MontoGravado2OtraMoneda,
  td_1.MontoGravado3OtraMoneda,
  ...
  tr1.numero AS NumeroDocumentoNCFModificado,
...
FROM
  dbo.cxcmovi2 AS cxc2
WITH
  (NOLOCK)
  LEFT OUTER JOIN dbo.sis_TipoNCF AS tn
WITH
  (NOLOCK) ON tn.Codigo = SUBSTRING(cxc2.ncf, 2, 2)
  LEFT OUTER JOIN dbo.empresa AS e
WITH
  (NOLOCK) ON cxc2.RNCEmisor = e.rnc
  LEFT OUTER JOIN dbo.cliente AS cl
WITH
  (NOLOCK) ON cl.cliente = cxc2.cliente
  LEFT OUTER JOIN td_1 ON td_1.numero = cxc2.documento  AND td_1.tipo = '05'
  LEFT JOIN TRANSA01 TR1 ON TRIM(TR1.documento) = TRIM(cxc2.documento) AND TR1.tipo = '05'
WHERE
  (cxc2.tipomovi IN ('03'))
  AND (cxc2.ncf IS NOT NULL)
  AND (cxc2.ncf <> '')
