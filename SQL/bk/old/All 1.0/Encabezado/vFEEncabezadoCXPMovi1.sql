...
SELECT
  ...
  */
  ...
  e.RazonSocialEmisor,
  CAST('' AS CHAR(100)) AS NombreComercial,
  CAST('' AS CHAR(100)) AS Sucursal,
  CAST('' AS CHAR(100)) AS DireccionEmisor,
  CAST('' AS CHAR(100)) AS  Municipio,
  CAST('' AS CHAR(100)) AS Provincia,
  CAST('' AS CHAR(100)) AS CorreoEmisor,
  CAST('' AS CHAR(100)) AS WebSite,
  CAST('' AS CHAR(100)) AS ActividadEconomica,
  CAST('' AS CHAR(100)) AS TelefonoEmisor1,
  ...
  cxp.EstadoFiscal,
  CASE
    WHEN RIGHT(cxp.ncf, 2) = '32'
    AND cxp.monto < 250000 THEN CONCAT(
      ...
      cxp.monto,
      '&CodigoSeguridad=',
      [dbo].[FNCambiaHexadecimal] (TRIM(cxp.CodigoSeguridad))
    )
    WHEN RIGHT(cxp.ncf, 2) = '47' THEN CONCAT(
      'https://ecf.dgii.gov.do/ecf/ConsultaTimbre?RncEmisor=',
      TRIM(cxp.rncemisor),
      '&ENCF=',
      TRIM(cxp.ncf),
      '&FechaEmision=',
      dbo.FNFechaDMY (cxp.fecha),
      '&MontoTotal=',
      cxp.monto,
      '&FechaFirma=',
      REPLACE(TRIM(cxp.FechaFirma), ' ', '%20'),
      '&CodigoSeguridad=',
      [dbo].[FNCambiaHexadecimal] (cxp.CodigoSeguridad)
    )
    ELSE CONCAT(
      'https://ecf.dgii.gov.do/ecf/ConsultaTimbre?RncEmisor=',
      TRIM(cxp.rncemisor),
