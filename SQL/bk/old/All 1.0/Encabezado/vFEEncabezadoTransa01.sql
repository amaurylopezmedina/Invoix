CREATE OR ALTER VIEW   vFEEncabezadoTransa01 AS 
...
WITH
  ...
  td_1 AS (
    ...
    WITH
      (NOLOCK)
      JOIN Transa01 tr ON tr.numero = td.numero
      AND tr.tipo = td.tipo
      CROSS JOIN e e
    WHERE
      td.tipo IN ('03', '04', '33', '34', '17')
      AND (tr.EstadoFiscal IS NOT NULL)
      AND isnull (tr.ncf, '') <> ''
    GROUP BY
      td.numero,
      td.tipo
  ) --Datos de las tablas para construir las Facturas y NC
SELECT
  ...
  (
    ...
    CASE
      WHEN tr.tipo = '03'
      OR tr.tipo = '33'
      OR tr.tipo = '17' THEN 0
      WHEN tr.tipo = '04'
      OR tr.tipo = '34' THEN tr.dia
    END
  ) AS TerminoPagoN,
    ...
    CASE
      ...
      OR tr.tipo = '17' THEN ''
      WHEN tr.tipo = '04'
      OR tr.tipo = '34' THEN CAST(tr.dia AS char (3)) + ' DIAS'
    END
  ) AS  TerminoPago,
  
  
  tr.almacen AS Almacen,
  /*(CASE WHEN tr.tipo = '17' THEN 1 ELSE 0 END) */
  ...
  (
    CASE
      WHEN tr.tipo = '03'
      OR tr.tipo = '33'
      OR tr.tipo = '17' THEN 1
      WHEN tr.tipo = '04'
      OR tr.tipo = '34' THEN 2
    END
  ) AS TipoPago,
  (
    CASE
      WHEN tr.tipo = '03'
      OR tr.tipo = '33'
      OR tr.tipo = '17' THEN 'CONTADO'
      WHEN tr.tipo = '04'
      OR tr.tipo = '34' THEN 'CREDITO'
    END
  ) AS TipoPagoL,
  ...
  (
    CASE
      WHEN COALESCE(tr.tasa, 1.00) = 1 THEN tr.monto * 1
      ELSE tr.monto * tr.tasa
    END
  ) AS MontoTotal,
  ...
  (
    CASE
      WHEN tr.tipo = '33'
      OR tr.tipo = '34' THEN 'USD'
      WHEN tr.tipo = '03'
      OR tr.tipo = '04'
      OR tr.tipo = '17' THEN 'DOP'
      ELSE 'DOP'
    END
  ) AS TipoMoneda,
  ...
  (
    CASE
      WHEN tr.tipo = '33'
      OR tr.tipo = '34' THEN 'DOLAR ESTADOUNIDENSE'
      WHEN tr.tipo = '03'
      OR tr.tipo = '04'
      OR tr.tipo = '17' THEN 'PESO DOMINICANO'
      ELSE 'PESO DOMINICANO'
    END
  ) AS TipoMonedaL,
  --Tipo de Cambio (Tasa)
  COALESCE(tr.tasa, 1.00) AS TipoCambio,
  ...
  (
    CASE
      WHEN tr.tipo = '17' THEN tr.observa
      ELSE CAST('' AS CHAR(100))
    END
  ...
  tr.EstadoFiscal,
    ...
    WHEN SUBSTRING(tr.ncf, 2, 2)  = '47' THEN CONCAT(
...
WITH
  (NOLOCK) ON c.cliente = tr.cliente
  LEFT OUTER JOIN dbo.empresa AS e
WITH
  (NOLOCK) ON tr.RNCEmisor = e.rnc
  LEFT OUTER JOIN dbo.cliente AS cl
WITH
  (NOLOCK) ON cl.cliente = tr.cliente
  LEFT OUTER JOIN dbo.sis_TipoNCF AS tn
WITH
  (NOLOCK) ON tn.Codigo = SUBSTRING(tr.ncf, 2, 2)
  LEFT OUTER JOIN dbo.Transa01 AS tr1
WITH
  (nolock) ON tr1.ncf = tr.ncf1
  LEFT OUTER JOIN td_1 ON td_1.numero = tr.numero
  AND td_1.tipo = tr.tipo
WHERE
