CREATE OR ALTER VIEW  vFEEncabezadoCXCMovi1Devolucion AS --CXCMovi1
WITH
  ...
  td_1 AS (
    SELECT
      ...
      MAX(td.tipo) AS tipo,
      MAX(cxc.RNCEmisor) AS RNCEmisor,
      MAX(cxc.ncf) AS ncf,
      MAX(COALESCE(cxc.tasa, 1.00)) AS tasa,
    ...
    WITH
      ...
      AND td.tipo = '05'
      AND cxc.tipomovi = '03'
      LEFT OUTER JOIN dbo.empresa AS e ON e.rnc = cxc.RNCEmisor
    WHERE
...
SELECT
  ...
  (
    CASE
      WHEN cxc.tipomovi = '05' THEN NULL
      ELSE cxc.vence
    END
  ) AS FechaLimitePago,
  ...
  */
    (
	CASE
      WHEN SUBSTRING(cxc.ncf, 2, 2) = '34' AND DATEDIFF(DAY, tr1.fecha, cxc.fecha) >30 THEN 1
	  ELSE 0
    END
  ) AS IndicadorNotaCredito,
  ...
  (
    CASE
      ...
      WHEN cxc.tipomovi = '03'
      OR cxc.tipomovi = '02' THEN 'CREDITO'
    END
  ) AS TipoPagoL,
...
WITH
  ...
  LEFT OUTER JOIN td_1 ON td_1.numero = cxc.documento  AND td_1.tipo = '05'
  LEFT JOIN TRANSA01 TR1 ON TRIM(TR1.documento) = TRIM(CXC.documento) AND TR1.tipo = '05'
WHERE
  (cxc.tipomovi IN ('03'))
  AND (cxc.ncf IS NOT NULL)
  AND (cxc.ncf <> '')
...
--  Select * from cxcmovi1 where documento = 'A00000000490' and tipomovi = '03'
