...
create or alter view VFEDetalleCXCMovi2 as
SELECT
  ...
  (
    CASE
      WHEN COALESCE(cxc2.tasa, 0.00) = 0 THEN td.precio * 1
      ELSE td.precio * cxc2.tasa
    END
  ) AS PrecioUnitarioItem,
  (
    CASE
      WHEN COALESCE(cxc2.tasa, 0.00) = 0 THEN td.montodesc * 1
      ELSE td.montodesc * cxc2.tasa
    END
  ) AS DescuentoMonto,
  ...
  (
    CASE
      WHEN COALESCE(cxc2.tasa, 0.00) = 0 THEN td.montoitbis * 1
      ELSE td.montoitbis * cxc2.tasa
    END
  ) AS MontoITBIS,
  td.descuen AS TasaDescuento,
  NULL AS TipoSubDescuento1,
  ...
  (
    CASE
      WHEN COALESCE(cxc2.tasa, 0.00) = 0 THEN td.monto1 * 1
...
WHERE
  ...
  union all
  -- CxcMovi 1 movimiento 05 - nota de debito
    SELECT
    NULL AS NumeroLinea,
    ...
    (CASE WHEN ISNULL(cxc2.descuen,0) <> 0 THEN '$' ELSE NULL END) AS TipoSubDescuento1,
    NULL AS SubDescuentoPorcentaje1,
