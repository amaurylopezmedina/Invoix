--CXCMovi1
CREATE OR ALTER VIEW vFEEncabezadoCXCMovi1 AS 
WITH
  td_1 AS (
    SELECT
...
SELECT
  ...
  */
    (
	CASE
      WHEN SUBSTRING(cxc.ncf, 2, 2) = '34' AND DATEDIFF(DAY, cxcd.fecha1, cxc.fecha) >30 THEN 1
	  ELSE 0
    END
  ) AS IndicadorNotaCredito,
...
WITH
  ...
  LEFT OUTER JOIN cxcdetalle1 AS cxcd ON cxc.documento = cxcd.documento
  AND cxc.tipomovi = cxcd.tipomovi
  LEFT OUTER JOIN td_1 ON td_1.numero = cxc.documento
  AND td_1.tipo = '05'
WHERE
  (cxc.tipomovi IN ( '03'))
  AND (cxc.ncf IS NOT NULL)
