SELECT
  ...
  tr.dia AS TerminoPagoN,
  CAST(tr.dia AS char (3)) + ' Dias' AS TerminoPago,
  tr.almacen,
  (
    CASE
  ...
  tr.venname AS NombreVendedor,
  (
    CASE
      WHEN COALESCE(tr.tasa, 0.00) = 0 THEN tr.monto * 1
  ...
  COALESCE(tr.tasa, 0.00) AS TipoCambio,
...
FROM
  dbo.Transa01 AS tr
  LEFT OUTER JOIN dbo.cliente AS c ON c.cliente = tr.cliente
  LEFT OUTER JOIN dbo.empresa AS e ON tr.RNCEmisor = e.rnc
  LEFT OUTER JOIN dbo.cliente AS cl ON cl.cliente = tr.cliente
  LEFT OUTER JOIN dbo.sis_TipoNCF AS tn ON tn.Codigo = SUBSTRING(tr.ncf, 2, 2)
  LEFT OUTER JOIN (
    SELECT
      td.numero,
      td.fecha,
    ...
    GROUP BY
      ...
      td.itbis
  ) AS td_1 ON td_1.numero = tr.numero
