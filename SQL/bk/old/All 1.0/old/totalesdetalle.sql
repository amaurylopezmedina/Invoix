SELECT
    ...
    tr.ncf AS eNCF,
    tr.RNCEmisor,
    'Interna' AS TipoCodigo1,
    td.producto AS CodigoItem1,
    NULL AS TipoCodigo2,
    NULL AS CodigoItem2,
    NULL AS TipoCodigo3,
    NULL AS CodigoItem3,
    ...
    (
        CASE
            WHEN COALESCE(tr.tasa, 0.00) = 0 THEN td.precio * 1
            ELSE td.precio * tr.tasa
        END
    ) AS PrecioUnitarioItem,
    (
        CASE
            WHEN COALESCE(tr.tasa, 0.00) = 0 THEN td.montodesc * 1
    ...
    (
        CASE
            WHEN COALESCE(tr.tasa, 0.00) = 0 THEN td.montoitbis * 1
    ...
    (
        CASE
            WHEN COALESCE(tr.tasa, 0.00) = 0 THEN td.monto1 * 1
