...
--CXCMovi1
--CREATE OR ALTER VIEW vFEEncabezadoCXCMovi1Directa AS 
WITH
  e AS (
    SELECT
      itbisenprecio,
...
SELECT
  ...
  (
    CASE
      WHEN COALESCE(cxc.tasa, 1.00) = 0 THEN cxc.monto * 1
  ...
  COALESCE(cxc.tasa, 1.00) AS TipoCambio,
  ...
  (
    CASE
      WHEN COALESCE(cxc.tasa, 1.00) = 0 THEN 0
      ELSE cxc.monto
    END
