...
SELECT
 ...
 CAST('' AS CHAR(1)) AS RazonModificacion,
  ...
  /*(
    CASE
      WHEN COALESCE(tr.observa1, '') <> '' THEN tr.observa1 /*collate SQL_Latin1_General_CP1_CI_AS*/
      WHEN COALESCE(tr.observa3, '') <> '' THEN tr.observa3 /*collate SQL_Latin1_General_CP1_CI_AS*/
      ELSE ''
    END
  )*/ '' AS NotaPermanente,  
