...
def audit(
    ...
    # No auditar durante dry-run (pero sí loggear)
    if DRY_RUN:
        _log(
        ...
        return
    cursor.execute(
        f"INSERT INTO {AUDIT_TABLE}(ambiente, object_type, object_name, action, status, sql_text, old_def, new_def, error_text) "
        f"VALUES (?,?,?,?,?,?,?,?,?)",
        (
            AMBiente,
            object_type,
