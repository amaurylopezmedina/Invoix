...
def safe_print(msg: str) -> None:
    """Imprime mensajes protegiendo contra problemas de codificación en Windows."""
    try:
        print(msg)
...
HEADERS_PATH = Path(__file__).resolve().parents[1] / "config" / "encabezado_columns.json"
...
def load_header_columns() -> List[str]:
    # soporta tanto JSON válido como texto con comentarios // ...
    raw = HEADERS_PATH.read_text(encoding="utf-8")
    try:
...
def load_detalle_columns() -> Dict[str, dict]:
    ...
    detalle_path = Path(__file__).resolve().parents[1] / "config" / "detalle_columns.json"
...
def load_encabezado_columns() -> Dict[str, dict]:
    ...
    encabezado_path = Path(__file__).resolve().parents[1] / "config" / "encabezado_columns.json"
...
def sync_table_with_json(conn, table_name: str, json_path: str, drop_if_exists: bool = False) -> None:
    ...
    if result > 0 and drop_if_exists:
        safe_print(f"🗑️ Eliminando tabla existente {table_name} para recrearla con esquema actualizado...")
        try:
            conn.execute(text(f"DROP TABLE IF EXISTS [{table_name}]"))
            dropped = True
            result = 0
            safe_print(f"✅ Tabla {table_name} eliminada.")
        except Exception as exc:
            safe_print(f"⚠️ No se pudo eliminar {table_name}: {exc}")

    if result == 0:
        safe_print(f"⚙️ Creando tabla {table_name} según definición JSON...")
        sql_cols = []
        ...
        try:
            ...
            safe_print(f"✅ Tabla {table_name} creada correctamente con PK compuesta (ENCF, RNCEmisor).")
        except Exception as exc:
            safe_print(f"💥 Error al crear la tabla {table_name}: {exc}")
        return

    if not dropped:
        safe_print(f"🔍 Verificando columnas existentes en {table_name}...")
            ...
            if normalized_col in existing_cols:
                ...
                if mismatch:
                    try:
                        ...
                        altered += 1
                        safe_print(f"⚠️ Tipo ajustado: {normalized_col} → {sql_def}")
                    except Exception as exc:
                        safe_print(f"💥 Error ajustando columna {normalized_col}: {exc}")
            else:
                try:
                    conn.execute(text(f"ALTER TABLE {table_name} ADD [{normalized_col}] {sql_def}"))
                    added += 1
                    safe_print(f"➕ Columna agregada: {normalized_col} ({sql_def})")
                except Exception as exc:
                    safe_print(f"💥 Error agregando columna {normalized_col}: {exc}")

        safe_print(
            f"✅ Sincronización completada para {table_name}. Columnas nuevas: {added}, ajustadas: {altered}"
        )
...
def ensure_tables_exist(df: pd.DataFrame, recreate_mode: bool = False) -> List[str]:
    ...
    json_path = Path(__file__).resolve().parents[1] / "config" / "encabezado_columns.json"

    with eng_db.connect() as conn:
        if recreate_mode:
            # Modo RECREAR: eliminar tabla de encabezado y todas las de detalle
            safe_print("🔄 Modo RECREAR: eliminando tablas existentes...")
            ...
            for row in result:
                table_name = row[0]
                safe_print(f"🗑️ Eliminando tabla de detalle {table_name}...")
                try:
                    conn.execute(text(f"DROP TABLE IF EXISTS [{table_name}]"))
                except Exception as exc:
                    safe_print(f"⚠️ Error al eliminar tabla {table_name}: {exc}")
        else:
            # Modo AGREGAR: solo sincronizar estructura sin eliminar datos
            safe_print("➕ Modo AGREGAR: sincronizando estructura de tablas...")
            ...
            if not _table_exists(conn, detalle_table):
                safe_print(f"ℹ️ Tabla de detalle {detalle_table} no existe, será creada al insertar datos")
...
def split_dataframe(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    ...
    safe_print(f"📋 Columnas válidas de encabezado en JSON: {len(header_cols_meta)}")
    safe_print(f"📋 Columnas base válidas de detalle en JSON: {len(valid_detalle_base_cols)}")
    ...
    safe_print(f"📊 Columnas de detalle a importar: {len(cols_det_all)}")
    safe_print(f"📊 Columnas de encabezado a importar: {len(cols_head_all)}")
    safe_print(f"📊 Columnas ignoradas (no en JSON): {len(cols_ignoradas)}")
    safe_print(f"📊 Total columnas en Excel: {len(df.columns)}")
    
    if cols_ignoradas:
        safe_print(f"⚠️ Columnas ignoradas: {cols_ignoradas[:10]}{'...' if len(cols_ignoradas) > 10 else ''}")
    ...
    if len(cols_head_all) > MAX_COLS_ENCABEZADO:
        safe_print(f"⚠️ Encabezado tiene {len(cols_head_all)} columnas, se particionará en múltiples tablas")
        ...
        safe_print(f"📋 Encabezado particionado en {1 + len(head_extra_tables)} tablas")
    ...
    safe_print(f"📊 Transponiendo {len(cols_det_all)} columnas de detalle a formato vertical...")
    ...
    safe_print(f"📋 Encontrados {len(indices_detalle)} ítems de detalle (índices: {min(indices_detalle) if indices_detalle else 0} a {max(indices_detalle) if indices_detalle else 0})")
    
    # Usar solo los nombres base definidos en JSON
    base_names_detalle = sorted(valid_detalle_base_cols)
    safe_print(f"📋 Campos por ítem (definidos en JSON): {len(base_names_detalle)}")
    safe_print(f"📋 Filas esperadas: {len(df)} facturas × {len(indices_detalle)} ítems = {len(df) * len(indices_detalle)} filas")
    ...
    safe_print(f"✅ Detalle transpuesto: {filas_despues} filas con CantidadItem válida ({filas_eliminadas} filas sin cantidad eliminadas)")
    ...
    if head_extra_tables:
        safe_print(f"✅ {len(head_extra_tables)} tablas de encabezado adicionales preparadas.")
...
def _filter_duplicates(df: pd.DataFrame, table_name: str, engine) -> pd.DataFrame:
    ...
    if 'ENCF' not in df.columns or 'RNCEmisor' not in df.columns:
        safe_print("⚠️ No se puede validar duplicados: faltan columnas ENCF o RNCEmisor")
        return df
    ...
    try:
        with engine.connect() as conn:
            ...
            if not _table_exists(conn, table_name):
                safe_print(f"ℹ️ Tabla {table_name} no existe, no hay duplicados que validar")
                return df
            
            # Obtener pares ENCF+RNCEmisor únicos del DataFrame
            unique_pairs = df[['ENCF', 'RNCEmisor']].drop_duplicates()
            total_original = len(df)
            
            # Crear lista de condiciones para buscar en la BD
            conditions = []
            for _, row in unique_pairs.iterrows():
                encf = str(row['ENCF']).replace("'", "''")
            ...
            if len(existing) == 0:
                safe_print("✅ No se encontraron duplicados")
                return df
            
            # Crear una columna combinada para hacer el merge
            df['_key'] = df['ENCF'].astype(str) + '|' + df['RNCEmisor'].astype(str)
            ...
            if duplicates_count > 0:
                safe_print(f"⚠️ Se encontraron {duplicates_count} registros duplicados que serán omitidos")
                safe_print(f"✅ Se insertarán {len(df_filtered)} registros nuevos")
            
            return df_filtered
            
    except Exception as e:
        safe_print(f"⚠️ Error al validar duplicados: {e}")
        safe_print("ℹ️ Continuando sin validación de duplicados...")
        return df
...
def insert_dataframes(
    ...
    safe_print("🔧 Normalizando nombres de columnas...")
    df_head = _normalize_column_names(df_head)

    # Validar duplicados si está habilitado
    if validate_duplicates and 'ENCF' in df_head.columns and 'RNCEmisor' in df_head.columns:
        safe_print("🔍 Validando duplicados antes de insertar...")
        df_head = _filter_duplicates(df_head, cfg["table_encabezado"], eng_db)
        if len(df_head) == 0:
            safe_print("⚠️ Todos los registros ya existen en la base de datos. No se insertará nada.")
            return 0, {}
    ...
    try:
        ...
        safe_print(f"✅ Encabezado: {head_count} filas insertadas (agregadas a tabla existente).")
    except Exception as exc:
    ...
    if validate_duplicates and len(df_head) > 0 and 'eNCF' in df_head.columns and 'RNCEmisor' in df_head.columns:
        safe_print("🔍 Filtrando detalles para que coincidan con encabezados insertados...")
        ...
        for table_name in list(det_tables.keys()):
            ...
            if 'eNCF' in df_det.columns and 'RNCEmisor' in df_det.columns:
                ...
                if filtered_count > 0:
                    safe_print(f"   ℹ️ {table_name}: {filtered_count} filas filtradas, {len(df_det_filtered)} filas a insertar")
    ...
    safe_print(f"⚙️ Creando {len(det_tables)} tablas...")
    with eng_db.connect() as conn:
        for table_name, df_det in det_tables.items():
            if not _table_exists(conn, table_name):
                _create_table(conn, table_name, list(df_det.columns))
    safe_print(f"✅ Todas las tablas creadas.")

    # Insertar datos en todas las tablas
    safe_print(f"⬆️ Insertando datos en {len(det_tables)} tablas...")
    table_count = 0
    
    for table_name, df_det in det_tables.items():
        if len(df_det) == 0:
            safe_print(f"   ℹ️ {table_name}: Sin datos para insertar (todos filtrados)")
        ...
        try:
            ...
            table_count += 1
            if table_count % 10 == 0:
                safe_print(f"   📊 Progreso: {table_count}/{len(det_tables)} tablas insertadas...")
        except Exception as exc:
    ...
    safe_print(f"✅ Todas las tablas insertadas.")
